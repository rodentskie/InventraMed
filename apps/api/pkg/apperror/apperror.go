...
// Medicine conflicts. All wrap ErrConflict, so errors.Is(err, ErrConflict)
// matches any of them while callers can still tell them apart.
var (
	...
	// ErrLocationTaken is returned when another active medicine already sits
	// in the tray compartment.
	ErrLocationTaken = fmt.Errorf("medicine location already taken: %w", ErrConflict)
